...
/**
 * Middleware para validar datos de Acronics DESPUÉS de procesar multipart/form-data
 * Evita crear registros si los datos requeridos son inválidos
 */
export const validarAcronics = async (req, res, next) => {
	try {
		...
		console.error("validarAcronics:", error);
		return res.status(500).json({
			message: "Error al validar Acronics",
			error: error.message
		});
	}
};
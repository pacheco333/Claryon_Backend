...
const storage = multer.diskStorage({
	destination: async (req, file, cb) => {
		try {
			...
			const uploadsDir = path.join(__dirname, `../uploads/empleados/${empleadoId}/glpi`);
	...
	filename: (req, file, cb) => {
		// Formato: glpi-{id}-foto{numero}-{timestamp}.ext
		const glpiId = req.params.id || 'temp';
		const fieldName = file.fieldname; // 'fotografia_1' o 'fotografia_2'
		const fotoNum = fieldName === 'fotografia_2' ? '2' : '1';
		const timestamp = Date.now();
		const ext = path.extname(file.originalname);

		cb(null, `glpi-${glpiId}-foto${fotoNum}-${timestamp}${ext}`);
	}
});
...
// Configurar multer para GLPI
const uploadGlpi = multer({
	storage: storage,
...
export const upload = uploadGlpi;

...
const storage = multer.diskStorage({
    destination: async (req, file, cb) => {
        try {
            ...
            const uploadsDir = path.join(__dirname, `../uploads/empleados/${empleadoId}/dispositivos`);
    ...
    filename: (req, file, cb) => {
        // Formato: dispositivo-{deviceId}-foto{numero}-{timestamp}.ext
        // Se extrae el deviceId de los parámetros de la URL
        const deviceId = req.params.id || 'temp';
        const fieldName = file.fieldname; // 'fotografia_1' o 'fotografia_2'
        const fotoNum = fieldName === 'fotografia_2' ? '2' : '1';
        const timestamp = Date.now();
        const ext = path.extname(file.originalname);
        
        cb(null, `dispositivo-${deviceId}-foto${fotoNum}-${timestamp}${ext}`);
    }
});
...
// Configurar multer para dispositivos
const uploadDispositivos = multer({
    storage: storage,
...
export const upload = uploadDispositivos;
